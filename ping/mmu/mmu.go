...
func makeIdentityPage(pid uint64, vAddr uint64, pageSize uint64) vm.Page {
	return vm.Page{
		PID:         vm.PID(pid),
		VAddr:       vAddr,
		PAddr:       vAddr,
...
func (m *middleware) finalizePageWalk(
	...
	if !found {
		identity := makeIdentityPage(uint64(req.PID), req.VAddr, 1<<m.log2PageSize)
		m.pageTable.Insert(identity)
		page = identity
	}
